...
// Uses HFG_BASE computed by mobile-fixes.js (loaded before this file).
// Falls back to relative paths when HFG_BASE is not set.
function _getJsonPath() {
    // Prefer the base computed by mobile-fixes.js
    if (window.HFG_BASE) {
    ...
    if (window.location.hostname.includes('github.io')) {
        ...
        return window.location.origin + repoBase + '/products/products.json';
    }
    if (path.includes('/pages/')) return '../products/products.json';
...
async function renderProductGrid(container, filter) {
    ...
    container.innerHTML = list.map(p => {
        ...
        const descText = desc.length === 80 ? desc + '...' : desc;
        const startPrice = getStartingPrice(p);
        const priceHTML = startPrice
            ? `<p class="font-label-bold mb-4" style="font-size:13px;color:#1b1b1b">Starting from <span style="color:#E10600">${formatPrice(startPrice)}</span></p>`
            : '';

        return `
        <div class="group border border-outline-variant bg-surface-container-lowest overflow-hidden flex flex-col" data-reveal>
            <a href="${pagePrefix}product.html?id=${encodeURIComponent(p.id)}" class="relative aspect-square overflow-hidden block bg-white">
                <img
                    src="${img}"
                    alt="${p.name}"
                    loading="lazy"
            ...
            <div class="p-5 flex flex-col flex-grow">
                ...
                <p class="font-body-md text-neutral-gray mb-2" style="font-size:12px;font-weight:400;letter-spacing:0.04em">${p.categoryName || p.category || ''}</p>
                ${priceHTML}
                <div class="mt-auto flex flex-col gap-2">
                    <a href="${pagePrefix}product.html?id=${encodeURIComponent(p.id)}"
                        class="w-full bg-accent-red text-on-primary py-3 font-label-bold uppercase flex items-center justify-center gap-2 hover:bg-primary transition-colors text-center text-sm">
                        <span class="material-symbols-outlined text-[16px]">shopping_bag</span> Order Sample
                    </a>
                    <a href="${rootPrefix}custom-sample.html?productId=${encodeURIComponent(p.id)}"
                        class="w-full border-2 border-primary py-3 font-label-bold uppercase flex items-center justify-center gap-2 hover:bg-primary hover:text-on-primary transition-colors text-center text-sm">
                        <span class="material-symbols-outlined text-[16px]">edit</span> Customize This
                    </a>
                </div>
            </div>
        </div>`;
